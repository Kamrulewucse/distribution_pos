...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box">
                <div class="box-body">
                    <form action="{{ route('report.sr_sale') }}">
                        <div class="row">
                            <div class="col-md-6">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="sr">
                                        <option value="">All SR</option>

                                        @foreach($srs as $sr)
                                            <option value="{{ $sr->id }}" {{ request()->get('sr') == $sr->id ? 'selected' : '' }}>{{ $sr->name.' - '.$sr->mobile_no }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                    ...
                                    <td>
                                        @if ($order->sale_type == 2)
                                            @if(auth()->user()->role == \App\Enumeration\Role::$ADMIN)
                                                <a class="btn btn-danger btn-sm btn-approve" data-id = {{$order->id}} id="btn-approve">Approve</a>
                                            @endif
                                        <a class="btn btn-primary btn-sm" href="{{ route('sale_receipt.details', ['order' => $order->id]) }}">View Invoice</a>
                                        @else
